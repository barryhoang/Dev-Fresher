...
public class SoonsoonData
{
    ...
    public void SavePackageData()
    {
        ...
        #if UNITY_EDITOR
        for( var i = 0 ; i < SpumManager._textureList.Count;i++)
        {
            Dictionary<string,bool> tList = new Dictionary<string,bool>();
            for(var j = 0 ; j < SpumManager._textureList[i]._packageList.Count;j++)
            {
                tList.Add(SpumManager._textureList[i]._packageNameList[j],SpumManager._textureList[i]._packageList[j]);
            }
    ...
    public void LoadPackageData()
    {
        ...
        for( var i = 0 ; i < SpumManager._textureList.Count;i++)
        {
            ...
            for(var j = 0 ; j < SpumManager._textureList[i]._packageList.Count;j++)
            {
                string tName = SpumManager._textureList[i]._packageNameList[j];
            ...
            SpumManager._textureList[i]._packageList.Clear();

            for ( var j = 0 ; j < tList.Count;j++)
            {
                 SpumManager._textureList[i]._packageList.Add(tList[j]);
            } 
        }

        SpumManager.LinkPackageList();
        #endif
    }
}
